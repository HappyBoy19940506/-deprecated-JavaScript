...
+ 所有的JS文件都写在 **HTML文件里面的**\<script>标签里面
    ...
    + 要看作用性，一般来说有些必备的js放在head里，有些自己写的响应效果的放在body最后

+ 可以使用同一类型的多个script标签， 多个script标签时，自上而下依次执行。

+ ~~一个html只能存在一种类型的script标签，要么执行script标签内部的，要么执行src外部引入的，不能同时都执行。~~ :x: :x: :x:
...
4. ```js
    do you feel the power of code ?121221121212121212121121211212212
    ```
